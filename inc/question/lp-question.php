...
class LP_Question{
    ...
    function admin_script(){
        ...
        if( !in_array( $post_type, array( LP()->question_post_type, LP()->quiz_post_type, LP()->lesson_post_type ) ) ) return;
        if( empty( $post->ID ) || $wp_query->is_archive ) return;
        wp_enqueue_style('lpr-question', LP_PLUGIN_URL . '/inc/question-type/assets/css/admin.question.css');
        wp_enqueue_script('lpr-question', LP_PLUGIN_URL . '/inc/question-type/assets/js/admin.question.js');
    }
    ...
    <div class="lpr-question lpr-question-<?php echo preg_replace('!_!', '-', $this->get_type() );?>" data-id="<?php echo $this->get('ID');?>">
        <div class="lpr-question-head">
            ...
            <select name="lpr_question[<?php echo $post_id;?>][type]" data-type="<?php echo $this->get_type();?>">
                ...
                <?php $question = LP_Question::instance( $type );?>
                    <?php if( $question ){?>
    ...
    function get_type( $slug = false ){
        $type = strtolower( preg_replace( '!LP_Question_!', '', get_class( $this ) ) );
     ...
     * If the first param is a string ( type of question such as true_or_false ) then return the instance of class LP_Question_True_Or_False
    ...
    static function instance( $id_or_type = null, $options = null ){
        ...
        $class_name = 'LP_Question_' . ucfirst( preg_replace_callback( '!(_[a-z])!', create_function ('$matches', 'return strtolower($matches[1]);'), $type ) );
        $class_instance = false;

        if( !class_exists( $class_name ) ){
            $paths = array(
                LP_PLUGIN_PATH . '/inc/question-type'
            );
            ...
            if( $paths ) foreach( $paths as $path ){
                if( is_file( $path ) ){
                    ...
                    $file = $path . '/class-lp-question-type-' . preg_replace( '!_!', '-', $type ) . '.php';
                }
                if( file_exists( $file ) ){
                    require_once( $file );
                }
...
function lpr_get_question( $type, $options = array() ){
    return LP_Question::instance( $type, $options );
}
...
function lpr_question_scripts(){
    if( !in_array( get_post_type(), array(LP()->question_post_type, LP()->quiz_post_type) ) ) return;
