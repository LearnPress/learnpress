        ...
        <div id="lpr-question-options-wrap">
            ...
            <?php if( $questions ):?>
                <?php foreach( $questions as $type ):?>
                    <option value="<?php echo $type;?>" <?php selected( ( isset( $question['type'] ) && $type == $question['type'] ) ? 1 : 0, 1);?>><?php echo LP_Question::instance( $type )->get_name();?></option>
                <?php endforeach;?>
            <?php endif;?>
            </select>
            <div class="lpr-question-settings">
            <?php if( isset( $question['type'] ) && $q = LP_Question::instance( $question['type'], array('ID' => $post_id) ) ){?>
