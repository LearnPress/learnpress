...
function lp_setup_course_data( $post ) {
	...
	if ( empty( $post->post_type ) || !in_array( $post->post_type, array( 'lpr_course' ) ) )
		return;
...
function lp_setup_quiz_data( $post ) {
	...
	if ( empty( $post->post_type ) || !in_array( $post->post_type, array( 'lpr_quiz' ) ) )
		return;
...
if ( !function_exists( 'learn_press_frontend_single_quiz_scripts' ) ) {
	...
	function learn_press_frontend_single_quiz_scripts() {
		if ( is_single() && 'lpr_quiz' == get_post_type() ) {
			...
			wp_enqueue_script( 'single-quiz', LPR_PLUGIN_URL . '/assets/js/single-quiz.js', array( 'jquery-cookie', 'backbone', 'wp-util' ) );
		}
	}
}
