...
class LP_Page_Controller {
	...
	/**
	 * Detect Course Builder URL path as a fallback when rewrite query vars are overridden.
	 *
	 * @return bool
	 */
	private function is_request_course_builder_path(): bool {
		...
		return $request_path === $page || 0 === strpos( $request_path, "{$page}/" );
	}

	/**
	 * Hydrate course builder query vars from request path when rewrite vars are missing.
	 *
	 * @param WP_Query $wp_query
	 *
	 * @return void
	 */
	private function hydrate_course_builder_query_vars_from_request_path( WP_Query $wp_query ): void {
		$segments = $this->get_request_course_builder_segments();
		if ( empty( $segments ) ) {
			return;
		}

		global $wp;
		$set_query_var = static function ( string $key, $value ) use ( $wp_query, $wp ): void {
		...
		$tab = sanitize_key( (string) $segments[0] );
		if ( '' !== $tab ) {
			$set_query_var( 'tab', $tab );
		}

		$arg_1 = isset( $segments[1] ) ? (string) $segments[1] : '';
		if ( '' === $arg_1 ) {
			return;
		}

		if ( 'post-new' === $arg_1 ) {
			$set_query_var( 'post_id', 'post-new' );
			$section = isset( $segments[2] ) ? sanitize_key( (string) $segments[2] ) : '';
			if ( '' !== $section ) {
		...
		if ( 'page' === $arg_1 ) {
			$paged = isset( $segments[2] ) ? absint( $segments[2] ) : 0;
			if ( $paged > 0 ) {
		...
		if ( ctype_digit( $arg_1 ) ) {
			$post_id = absint( $arg_1 );
			if ( $post_id > 0 ) {
	...
	/**
	 * Get request path segments after `/course-builder/`.
	 *
	 * @return array<int, string>
	 */
	private function get_request_course_builder_segments(): array {
		$page = LP_Settings::get_option( 'course_builder', 'course-builder' );
		$page = trim( (string) $page, " \t\n\r\0\x0B/" );
		if ( '' === $page ) {
			return [];
		...
		if ( $request_path === $page || 0 !== strpos( $request_path, "{$page}/" ) ) {
			return [];
		}

		$sub_path = trim( substr( $request_path, strlen( $page ) + 1 ), '/' );
		if ( '' === $sub_path ) {
			return [];
		}
