...
if ( ! class_exists( 'LP_Course_Item' ) ) {
	...
	class LP_Course_Item extends LP_Abstract_Post_Data implements ArrayAccess {
		...
		public function is_preview( $context = 'display' ) {
			if ( '' === $this->_preview ) {
				...
				if ( $course = $this->get_course() ) {
					...
					if ( false === ( $cached = wp_cache_get( 'item-' . $user_id . '-' . $course->get_id() . '-' . $this->get_id(), 'learn-press/preview-items' ) ) ) {
						$user = learn_press_get_current_user();

						if ( $user->has_enrolled_course( $course->get_id() ) ) {
							$is_preview = false;
						}
						wp_cache_set( 'item-' . $user_id . '-' . $course->get_id() . '-' . $this->get_id(), $is_preview ? 'yes' : 'no', 'learn-press/preview-items' );
					} else {
		...
		public function get_format() {
			...
			$format = get_post_meta( $this->get_id(), 'post_format', true );// ( false !== ( $format = wp_cache_get( 'item-format-' . $this->get_id(), 'learn-press/item-formats' ) ) ) ? $format : get_post_format( $this->get_id() );
		...
		public function get_class( $more = '', $user_id = 0 ) {
			...
			if ( false === ( $classes = wp_cache_get( 'item-' . $user_id . '-' . $this->get_id(), 'learn-press/post-classes' ) ) ) {
				if ( ! $user_id ) {
					$user_id = get_current_user_id();
				}
				...
				wp_cache_set( 'item-' . $user_id . '-' . $this->get_id(), $classes, 'learn-press/post-classes' );
			}
		...
		public static function get_item( $post, $course = null ) {
			...
			if ( false === ( $item = wp_cache_get( $item_id, 'learn-press/object-classes' ) ) ) {
		...
		public function is_blocked( $course_id = 0, $user_id = 0 ) {
			...
			if ( false === ( $blocked_items = wp_cache_get( $key, 'learn-press/blocked-items' ) ) ) {
		...
		protected function _parse_item_block_status( $course_id, $user_id, $cache_key ) {
			...
			wp_cache_set( $cache_key, $blocked_items, 'learn-press/blocked-items' );
