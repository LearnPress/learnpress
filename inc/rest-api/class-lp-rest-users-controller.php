...
class LP_REST_Users_Controller extends LP_Abstract_REST_Controller {
				...
				if ( $question_ids ) {
					foreach ( $question_ids as $id ) {
						...
						$results['questions'][ $id ] = array(
							...
							'options'     => learn_press_get_question_options_for_js( $question, array( 'include_is_true' => true, 'fib_answer' => $answered[ $id ] ) ),
						);
					}
				}
			}
	...
	public function check_answer( $request ) {
		...
		$response = array(
			'explanation' => $question->get_explanation(),
			'options'     => learn_press_get_question_options_for_js( $question, array( 'include_is_true' => true, 'fib_answer' => $answered ) ),
			'result'      => $checked,
		);
