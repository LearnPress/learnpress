...
if ( !class_exists( 'LP_Quiz_Post_Type' ) ) {
    ...
    final class LP_Quiz_Post_Type extends LP_Abstract_Post_Type {
        ...
        public function register() {
            register_post_type( LP_QUIZ_CPT, apply_filters( 'lp_quiz_post_type_args', array(
                'labels' => array(
        ...
        public function add_meta_boxes() {
            ...
            $meta_box = apply_filters(
                    'learn_press_quiz_question_meta_box_args', array(
        ...
        public function columns_head( $columns ) {
            ...
            if ( false !== $pos && !array_key_exists( LP_COURSE_CPT, $columns ) ) {
                $columns = array_merge(
                        array_slice( $columns, 0, $pos + 1 ), array(
        ...
        public function columns_content( $name, $post_id = 0 ) {
            ...
            switch ( $name ) {
                ...
                case 'num_of_question':
                    ...
                    printf(
                            '<span class="lp-label-counter" title="%s">%s</span>', ( $count ) ? sprintf( _nx( '%d question', '%d questions', $count, 'learnpress' ), $count ) : __( 'This quiz has got any questions', 'learnpress' ), $count
                    );
                    break;
                case 'duration':
        ...
        public function posts_where_paged( $where ) {
            ...
            if ( isset( $_GET['s'] ) ) {
                $s = $_GET['s'];
                $where = preg_replace(
                        "/\.post_content\s+LIKE\s*(\'[^\']+\')\s*\)/", " .post_content LIKE '%$s%' ) OR (c.post_title LIKE '%$s%' )", $where
                );
            }
