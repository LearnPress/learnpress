...
class AiModel {
	...
	public static function get_course_curriculum_prompt( $params ) {
		...
		if ( $quiz_number > 0 ) {
			$quiz_instruction = <<<TXT
			- Each section MUST include a "quizzes" array.
			- The "quizzes" array must contain exactly $quiz_number quiz objects.
			- Each quiz object MUST have:
			  - "quiz_title": a relevant title.
			  - "questions": an array of multiple-choice questions.
			...
			- Each question MUST include:
			  ...
			  - "correct_answer" (one correct option)
			TXT;
			  ...
			  "sections": [
			    {
			      ...
			      "quizzes": [
			        ...
			        {
			          "quiz_title": "Quiz 2 for Section 1",
			          "questions": [
			            {
			              "question_text": "Which principle was introduced in Lesson 1.2?",
			              "options": ["Principle A", "Principle B", "Principle C", "Principle D"],
			              "correct_answer": "Principle B"
			            }
			          ]
			        }
			      ]
...
TASK:
Generate a detailed course curriculum based on the context.
If quizzes are requested, create multiple quizzes per section.
The quizzes must be based only on the lessons of the same section.
...
CONSTRAINTS:
- The output MUST be a raw JSON object (no markdown, no explanations).
- Generate exactly $section_number sections.
- Each section must contain exactly $less_per_section lessons.
$quiz_instruction
- The language for all generated content MUST be: $language.
EOT;
