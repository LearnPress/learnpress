...
class MaterialFilesDB extends DataBase {
	...
	public function get_files( MaterialFilesFilter $filter, int &$total_rows = 0 ) {
		...
		if ( empty( $filter->collection ) ) {
			$filter->collection = $this->table_name;
		}
		...
		if ( isset( $filter->file_id ) ) {
			$filter->where[] = $this->wpdb->prepare( "AND $ca.file_id = %d", $filter->file_id );
		}

		if ( ! empty( $filter->file_ids ) ) {
			$ids             = array_map( 'absint', $filter->file_ids );
			$filter->where[] = "AND $ca.file_id IN (" . implode( ',', $ids ) . ')';
		}

		if ( isset( $filter->item_id ) ) {
			if ( $filter->include_course_items ) {
		...
		if ( ! empty( $filter->item_ids ) ) {
			$ids             = array_map( 'absint', $filter->item_ids );
			$filter->where[] = "AND $ca.item_id IN (" . implode( ',', $ids ) . ')';
		}
