		...
		try {
			...
			$html_content = apply_filters(
				...
				[
					...
					'title'             => sprintf(
						'<h3 class="wap-quiz-title"><button data-popup-quiz="%s">%s</button></h3>',
						$quiz_model->get_id(),
						$quiz['title']
					),
			...
			$html_action = apply_filters(
				...
				[
					...
					'edit'                        => sprintf(
						'<div class="quiz-action-editor"><button class="btn-edit-quiz quiz-edit-permalink" data-popup-quiz="%s">%s %s</button></div>',
						$quiz_model->get_id(),
