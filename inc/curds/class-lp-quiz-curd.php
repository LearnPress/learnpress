...
if ( ! function_exists( 'LP_Quiz_CURD' ) ) {
	...
	class LP_Quiz_CURD extends LP_Object_Data_CURD implements LP_Interface_CURD {
		...
		public function load( &$quiz ) {
			...
			$quiz->set_pagination( get_post_meta( $quiz->get_id(), '_lp_pagination', true ) );
			$quiz->set_review_questions( get_post_meta( $quiz->get_id(), '_lp_review', true ) );
			$quiz->set_show_correct_review( get_post_meta( $quiz->get_id(), '_lp_show_correct_review', true ) );
			$quiz->set_preview( get_post_meta( $quiz->get_id(), '_lp_preview', true ) );
			$quiz->set_minus_skip_questions( get_post_meta( $quiz->get_id(), '_lp_minus_skip_questions', true ) );
			$quiz->set_instant_check( get_post_meta( $quiz->get_id(), '_lp_instant_check', true ) );
			$quiz->set_negative_marking( get_post_meta( $quiz->get_id(), '_lp_negative_marking', true ) );
