...
jQuery( function( $ ) {
	...
	var cloneIndex = {
		...
		set( $inputs, index ) {
			...
			const $address = $inputs.filter( '.rwmb-map-goto-address-button' );
			if ( $address.length ) {
				const value = $address.attr( 'value' );
		 ...
		 * @return string
		 */
		replace( index, value, before, after, alternative, isEnd ) {
			before = before || '';
			after = after || '';
			...
			const regex = new RegExp( cloneIndex.escapeRegex( before ) + '(\\d+)' + cloneIndex.escapeRegex( after ) + end ),
		 ...
		 */
		nextIndex( $container ) {
			const nextIndex = $container.data( 'next-index' );
	...
	var cloneValue = {
		 ...
		 */
		resetToDefault() {
			const defaultValue = cloneValue.$field.data( 'default' );
	...
	function clone( $container ) {
		...
		const $inputs = $clone.find( inputSelectors );
	...
	function toggleAddButton( $container ) {
		const $button = $container.children( '.add-clone' ),
	...
	$( document )
		...
		.on( 'click', '.add-clone', function( e ) {
			...
			const $container = $( this ).closest( '.rwmb-input' );
			clone( $container );
