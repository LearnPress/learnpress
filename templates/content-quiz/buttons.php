...
<form class="quiz-buttons" method="post">
	...
	<?php if ( $user->has( 'quiz-status', array( 'started' ), $quiz->id ) ): ?>
		...
		<?php if ( !$user->has_checked_answer( $question->id, $quiz->id, $course->get_id() ) ): ?>
			<button
				class="button-check-answer"
				data-id="<?php esc_attr_e( $quiz->id ); ?>"
				data-action="check-answer"
				data-security="<?php esc_attr_e( wp_create_nonce( 'check-question-' . $user->get_id() . '-' . $course->get_id() . '-' . $quiz->id ) ); ?>">
				<?php esc_html_e( 'Check', 'learnpress' ); ?>
			</button>
			<button class="button-hint" data-security="<?php esc_attr_e( wp_create_nonce( 'get-question-hint-' . $user->get_id() . '-' . $course->get_id() . '-' . $quiz->id ) ); ?>"><?php esc_html_e( 'Hint', 'learnpress' ); ?></button>
		<?php endif; ?>
	<?php endif; ?>
	...
	<?php if ( $user->has( 'quiz-status', 'completed', $quiz->id ) ): ?>
		...
		<?php if ( $remain = $user->can( 'retake-quiz', $quiz->id ) ): ?>
			<button
				...
				data-id="<?php esc_attr_e( $quiz->id ); ?>"
				data-security="<?php esc_attr_e( wp_create_nonce( 'retake-quiz-' . $user->get_id() . '-' . $course->get_id() . '-' . $quiz->id ) ); ?>">
				<?php echo esc_html( sprintf( '%s (+%d)', __( 'Retake', 'learnpress' ), $remain ) ); ?>
			</button>
		<?php endif; ?>

	<?php elseif ( $user->can_do_quiz( $quiz->id, $course->get_id() ) ): ?>
		<button
			class="button-start-quiz"
			data-action="start-quiz"
			data-id="<?php esc_attr_e( $quiz->id ); ?>"
			data-security="<?php esc_attr_e( wp_create_nonce( 'start-quiz-' . $user->get_id() . '-' . $course->get_id() . '-' . $quiz->id ) ); ?>">
	...
	<?php if ( in_array( $status, array( 'started' ) ) ): ?>
		<button
			...
			data-id="<?php esc_attr_e( $quiz->id ); ?>"
			data-security="<?php esc_attr_e( wp_create_nonce( 'finish-quiz-' . $user->get_id() . '-' . $course->get_id() . '-' . $quiz->id ) ); ?>">
