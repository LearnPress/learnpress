...
<?php if ( $gateway instanceof LP_Gateway_Abstract ) : ?>
	...
	<?php if ( apply_filters( 'learn_press_display_payment_method', true, $gateway->id ) ) { ?>
		<li>
			<label>
				<input id="payment_method_<?php echo $gateway->id; ?>" type="radio" class="input-radio" <?php checked($selected, true);?> name="payment_method" value="<?php echo esc_attr( $gateway->id ); ?>" <?php checked( LP()->session->get( 'chosen_payment_method' ) == $gateway->id, true ); ?> data-order_button_text="<?php echo esc_attr( $gateway->order_button_text ); ?>" />
