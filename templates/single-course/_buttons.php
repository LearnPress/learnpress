...
<div class="learn-press-course-buttons">
	<?php do_action( 'learn_press_before_course_buttons', $course->get_id() ); ?>
	<?php
	...
	<?php else:
		...
		if ( $course->is_required_enroll() ) {
			...
			$in_cart       = learn_press_is_added_to_cart( $course->get_id() );
			...
			if ( $user->has( 'finished-course', $course->get_id() ) ): ?>
				<?php if ( $count = $user->can( 'retake-course', $course->get_id() ) ): ?>
					<button
						class="button button-retake-course"
						data-course_id="<?php echo esc_attr( $course->get_id() ); ?>"
						data-security="<?php echo esc_attr( wp_create_nonce( sprintf( 'learn-press-retake-course-%d-%d', $course->get_id(), $user->get_id() ) ) ); ?>">
			...
			# Enrolled Course
			# -------------------------------
			elseif ( $user->has( 'enrolled-course', $course->get_id() ) ): ?>
				<?php
				$can_finish = $user->can_finish_course( $course->get_id() );
				//if ( $can_finish ) {
				$finish_course_security = wp_create_nonce( sprintf( 'learn-press-finish-course-' . $course->get_id() . '-' . $user->get_id() ) );
				//} else {
			...
			<?php elseif ( $user->can( 'purchase-course', $course->get_id() ) ) : ?>
				...
				<form name="purchase-course" class="purchase-course" method="post" enctype="multipart/form-data">
					...
					<input type="hidden" name="purchase-course" value="<?php echo $course->get_id(); ?>" />
					<input type="hidden" value="user can purchase course" />
				</form>

			<?php elseif ( $user->can( 'enroll-course', $course->get_id() ) === 'enough' ) : ?>
				<p class="learn-press-message"><?php echo $notice_enough_student; ?></p>
			<?php else: ?>
				<?php $order_status = $user->get_order_status( $course->get_id() ); ?>
	...
	?>
	<?php do_action( 'learn_press_after_course_buttons', $course->get_id() ); ?>
</div>