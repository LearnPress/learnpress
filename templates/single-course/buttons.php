	...
	<?php else:
		...
		if ( $course->is_required_enroll() ) {
			...
			if ( $user->has( 'finished-course', $course->id ) ): ?>
				<?php if ( $count = $user->can( 'retake-course', $course->id ) ): ?>
                    <button
                            ...
                            data-security="<?php echo esc_attr( wp_create_nonce( sprintf( 'learn-press-retake-course-%d-%d', $course->id, $user->get_id() ) ) ); ?>">
            ...
            elseif ( $user->has( 'enrolled-course', $course->id ) ): ?>
				...
				//if ( $can_finish ) {
				$finish_course_security = wp_create_nonce( sprintf( 'learn-press-finish-course-' . $course->id . '-' . $user->get_id() ) );
				//} else {
