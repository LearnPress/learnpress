<?php
/**
 * Template for displaying cart form.
 *
 * This template can be overridden by copying it to yourtheme/learnpress/cart/form.php
 *
 * @author  ThimPress
...
?>
    ...
    <form method="post" name="lp-cart" class="lp-cart" action="<?php echo esc_url( $checkout_url ); ?>"
        ...
        <table class="learn-press-cart-table">
			...
			<?php foreach ( LP()->cart->get_items() as $cart_item ) {
				...
				if ( $_course && $_course->exists() && $cart_item['quantity'] > 0 && apply_filters( 'learn_press_cart_item_visible', true, $cart_item ) ) {
                    ...
                    <tr class="<?php echo esc_attr( apply_filters( 'learn_press_cart_item_class', 'cart_item', $cart_item ) ); ?>">
                        ...
                        <td class="course-name">
							...
							?>&nbsp;
							<?php
							echo apply_filters( 'learn_press_cart_item_remove_link',
								sprintf( '<a href="%s" class="remove" title="%s" data-course_id="%s">%s</a>', esc_url( add_query_arg( 'remove-cart-item', $_course_id ) ), __( 'Remove this course', 'learnpress' ), esc_attr( $_course_id ), __( 'Remove', 'learnpress' ) ), $cart_item );
							?>
                        </td>

                        <td class="course-price">
							<?php echo learn_press_format_price( apply_filters( 'learn_press_cart_item_price', $_course->price, $cart_item ), true ); ?>
                        </td>

                        <td class="course-total">
							<?php echo learn_press_format_price( apply_filters( 'learn_press_cart_item_subtotal', $cart_item['subtotal'], $cart_item ), true ); ?>
                        </td>
                    </tr>
					<?php
				}
