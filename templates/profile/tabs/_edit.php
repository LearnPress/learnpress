...
$profile_picture_type    = $user->get_data( 'profile_picture' ) ? 'picture' : 'gravatar';
...
$url_tab                 = learn_press_user_profile_link( $user->get_id(), $current );
    ...
    <div id="lp-user-profile-form" class="lp-user-profile-form">
        <form method="post" name="lp-edit-profile">
            ...
            <p class="submit update-profile">
                <input disabled="disabled" type="submit" name="submit" id="submit" class="button button-primary"
                       value="<?php _e( 'Update', 'learnpress' ); ?>"/>
            </p>
        </form>
    </div>
<?php
