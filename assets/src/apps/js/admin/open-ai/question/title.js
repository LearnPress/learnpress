...
const title = () => {
	modal = document.querySelector('#lp-ai-question-title-modal');
...
const addTitleBtn = () => {
	document.querySelector('body.post-type-lp_question #titlewrap').insertAdjacentHTML('afterend', `
	<button type="button" class="button" id="lp-edit-ai-question-title">` + __('Edit with AI', 'learnpress') + `</button>`);
};
...
const copyText = () => {
	document.addEventListener('click', function (event) {
		...
		const modal = target.closest('#lp-ai-question-title-modal');

		if (!modal) {
			return;
		}

		const titleItem = target.closest('.question-title-item');
...
const openModal = () => {
	document.addEventListener('click', function (event) {
		...
		if (target.id !== 'lp-edit-ai-question-title') {
			return;
		}
...
const closeModal = () => {
	...
	const handleClose = () => {
		const openModalBtn = document.querySelector('#lp-edit-ai-question-title');
	...
	document.addEventListener('mouseup', function (event) {
		...
		if (isMouseDownOnTarget) {
			if (target.classList.contains('close-btn') && target.closest('#lp-ai-question-title-modal')) {
				handleClose();
			}
...
const generate = () => {
	document.addEventListener('click', function (event) {
		const target = event.target;
		if (!['lp-generate-question-title-btn', 'lp-re-generate-question-title'].includes(target.getAttribute('id'))) {
			return;
		}
		...
		const titleOutputNode = modal.querySelector('.question-title-output');
		...
		const topicNode = contentNode.querySelector('#ai-question-title-field-topic');
		const goalNode = contentNode.querySelector('#ai-question-title-field-goal');
		const audienceNode = contentNode.querySelector('#ai-question-title-field-audience');
		const toneNode = contentNode.querySelector('#ai-question-title-field-tone');
		const langNode = contentNode.querySelector('#ai-question-title-field-language');
		const outputsNode = contentNode.querySelector('#ai-question-title-field-outputs');
		...
		if (target.getAttribute('id') === 'lp-re-generate-question-title') {
