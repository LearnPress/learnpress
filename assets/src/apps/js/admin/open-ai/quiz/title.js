...
const title = () => {
	modal = document.querySelector('#lp-ai-quiz-title-modal');
...
const addTitleBtn = () => {
	document.querySelector('body.post-type-lp_quiz #titlewrap').insertAdjacentHTML('afterend', `
	<button type="button" class="button" id="lp-edit-ai-quiz-title">` + __('Edit with AI', 'learnpress') + `</button>`);
};
...
const copyText = () => {
	document.addEventListener('click', function (event) {
		...
		const modal = target.closest('#lp-ai-quiz-title-modal');

		if (!modal) {
			return;
		}

		const titleItem = target.closest('.quiz-title-item');
...
const openModal = () => {
	document.addEventListener('click', function (event) {
		...
		if (target.id !== 'lp-edit-ai-quiz-title') {
			return;
		}
...
const closeModal = () => {
	...
	const handleClose = () => {
		const openModalBtn = document.querySelector('#lp-edit-ai-quiz-title');
	...
	document.addEventListener('mouseup', function (event) {
		...
		if (isMouseDownOnTarget) {
			if (target.classList.contains('close-btn') && target.closest('#lp-ai-quiz-title-modal')) {
				handleClose();
			}
...
const generate = () => {
	document.addEventListener('click', function (event) {
		const target = event.target;
		if (!['lp-generate-quiz-title-btn', 'lp-re-generate-quiz-title'].includes(target.getAttribute('id'))) {
			return;
		}
		...
		const titleOutputNode = modal.querySelector('.quiz-title-output');
		...
		const topicNode = contentNode.querySelector('#ai-quiz-title-field-topic');
		const goalNode = contentNode.querySelector('#ai-quiz-title-field-goal');
		const audienceNode = contentNode.querySelector('#ai-quiz-title-field-audience');
		const toneNode = contentNode.querySelector('#ai-quiz-title-field-tone');
		const langNode = contentNode.querySelector('#ai-quiz-title-field-language');
		const outputsNode = contentNode.querySelector('#ai-quiz-title-field-outputs');
		...
		if (target.getAttribute('id') === 'lp-re-generate-quiz-title') {
