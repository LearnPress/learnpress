...
const description = () => {
	modal = document.querySelector('#lp-ai-quiz-des-modal');
...
const addDesBtn = () => {
	document.querySelector('body.post-type-lp_quiz #wp-content-media-buttons').insertAdjacentHTML('beforeend', `
	<button type="button" class="button" id="lp-edit-ai-quiz-des">` + __('Edit with AI', 'learnpress') + `</button>`);
};
...
const copyText = () => {
	document.addEventListener('click', function (event) {
		...
		const modal = target.closest('#lp-ai-quiz-des-modal');

		if (!modal) {
			return;
		}

		const descriptionItem = target.closest('.quiz-des-item');
...
const openModal = () => {
	document.addEventListener('click', function (event) {
		...
		if (target.id !== 'lp-edit-ai-quiz-des') {
			return;
		}
...
const closeModal = () => {
	...
	const handleClose = () => {
		const openModalBtn = document.querySelector('#lp-edit-ai-quiz-des');
	...
	document.addEventListener('mouseup', function (event) {
		...
		if (isMouseDownOnTarget) {
			if (target.classList.contains('close-btn') && target.closest('#lp-ai-quiz-des-modal')) {
				handleClose();
			}
...
const generate = () => {
	document.addEventListener('click', function (event) {
		const target = event.target;
		if (!['lp-generate-quiz-des-btn', 'lp-re-generate-quiz-des'].includes(target.getAttribute('id'))) {
			return;
		}
		...
		const desOutputNode = modal.querySelector('.quiz-des-output');
		...
		const topicNode = contentNode.querySelector('#ai-quiz-des-field-topic');
		// const goalNode = contentNode.querySelector( '#ai-quiz-des-field-goal' );
		const audienceNode = contentNode.querySelector('#ai-quiz-des-field-audience');
		const toneNode = contentNode.querySelector('#ai-quiz-des-field-tone');
		const langNode = contentNode.querySelector('#ai-quiz-des-field-language');
		const outputsNode = contentNode.querySelector('#ai-quiz-des-field-outputs');
		...
		if (target.getAttribute('id') === 'lp-re-generate-quiz-des') {
