...
const togglePrompt = () => {
	document.addEventListener( 'click', function( event ) {
		...
		if ( ! target.closest( '#lp-ai-course-des-modal' ) ) {
			return;
		}
...
const closeModal = () => {
	document.addEventListener( 'click', function( event ) {
		...
		const modal = document.querySelector( '#lp-ai-course-des-modal' );
...
const generate = () => {
	document.addEventListener( 'click', function( event ) {
		const target = event.target;
		if ( target.getAttribute( 'id' ) !== 'lp-generate-course-des-btn' ) {
			return;
		}
		...
		wp.apiFetch( {
			path: '/lp/v1/open-ai/course-des', method: 'POST', data,
		} ).then( ( res ) => {
			if ( res.data.prompt ) {
