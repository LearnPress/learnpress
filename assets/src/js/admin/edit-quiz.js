...
class EditQuiz {
	...
	addQuestion( e, target ) {
		...
		if ( target.closest( `${ EditQuiz.selectors.elBtnAddQuestion }` ) ) {
			canHandle = true;
		} else if ( target.closest( `${ EditQuiz.selectors.elQuestionTitleNewInput }` ) && e.key === 'Enter' ) {
			canHandle = true;
		}
		...
		const callBack = {
			...
			completed: () => {
				...
				this.checkCanAddQuestion( e, elQuestionTitleNewInput );
			},
		};
	...
	events() {
		...
		document.addEventListener( 'keyup', ( e ) => {
			const target = e.target;
			this.changeTitleQuestion( e, target );
			this.checkCanAddQuestion( e, target );
