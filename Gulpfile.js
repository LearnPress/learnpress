 ...
 */
let currentVer = null;
const rootPath = '/Users/tu/Documents/foobla',
...
gulp.task( 'copy-tag', gulp.series( 'clr-tag', function() {
	const tagPath = svnTagsPath + '/' + getCurrentVer();
	mkdirp( tagPath );
	process.chdir( svnTrunkPath );
	const copyFiles = copySvnFiles;
...
gulp.task( 'release', gulp.series( 'copy-release', function() {
	...
	const zipPath = releasePath.replace( /learnpress/, '' );
...
/*** Compress Js - tungnx ***/
const scriptSrc = [ 'assets/src/js/admin/**/*.js' ];
const scriptDestination = 'assets/mini/js/admin';
