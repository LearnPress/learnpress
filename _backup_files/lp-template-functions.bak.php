...
/*******************************************************************************************/
/*******************************************************************************************/
/************************ above is the functions for version 1.0 ***************************/
/*******************************************************************************************/
/*******************************************************************************************/
 ...
 * @return LP_Quiz
 */
function lp_setup_quiz_data( $post ) {
	...
	if ( empty( $post->post_type ) || !in_array( $post->post_type, array( LP()->quiz_post_type ) ) )
		return;

	$GLOBALS['quiz'] = lp_get_quiz( $post );

	$user = learn_press_get_current_user();
	$user->set_quiz( $GLOBALS['quiz'] );

	return $GLOBALS['quiz'];
}

add_action( 'the_post', 'lp_setup_quiz_data' );
...
if ( !function_exists( 'learn_press_course_percentage' ) ) {
	...
	function learn_press_course_percentage() {
		if ( learn_press_user_has_completed_quiz( null, 1183 ) ) {
			learn_press_get_template( 'single-course/course-result.php' );
		}
	}
}
...
if ( !function_exists( 'learn_press_frontend_single_quiz_scripts' ) ) {
	...
	function learn_press_frontend_single_quiz_scripts() {
		if ( is_single() && LP()->quiz_post_type == get_post_type() ) {
...
if ( !function_exists( 'learn_press_course_finished_message' ) ) {
	...
	function learn_press_course_finished_message() {
		if ( learn_press_user_has_finished_course() ) {
			...
			learn_press_get_template( 'single-course/course-result.php', array( 'quiz_id' => $quiz_id ) );
		}
	}
}
...
if ( !function_exists( 'learn_press_course_retake_button' ) ) {
	...
	function learn_press_course_retake_button() {
		if ( learn_press_user_has_finished_course() && learn_press_user_can_retake_course() ) {
			learn_press_get_template( 'single-course/button-retake.php' );
		}
	}
}
...
if ( !function_exists( 'learn_press_course_content_summary' ) ) {
	...
	function learn_press_course_content_summary() {
		...
		if ( $lesson_id && ( LP()->lesson_post_type == get_post_type( $lesson_id ) ) && ( $lesson = get_post( $lesson_id ) ) ) {
...
function learn_press_course_lesson_quiz_before_title( $lesson_or_quiz, $enrolled ) {
	...
	if ( !$enrolled ) {
		if ( LP()->quiz_post_type == get_post_type( $lesson_or_quiz ) ) {
...
function learn_press_quick_lesson_link_shortcode( $atts, $content = null ) {
	if ( LP()->lesson_post_type == get_post_type() ) {
		...
		if ( ( LP()->lesson_post_type == get_post_type( $atts['id'] ) ) && ( $course_id = learn_press_get_course_by_lesson( $atts['id'] ) ) ) {
 ...
 * @return mixed
 */
function learn_press_js_page_title( $title, $sep ) {
	if ( is_feed() || ( LP()->course_post_type != get_post_type() ) || empty( $_REQUEST['lesson'] ) ) {
		return $title;
	}
...
add_filter( 'wp_title', 'learn_press_js_page_title', 10, 2 );
...
function learn_press_add_buttons( $plugin_array ) {
	$plugin_array['embed'] = LP_PLUGIN_URL . '/assets/js/learnpress-embed-button.js';
